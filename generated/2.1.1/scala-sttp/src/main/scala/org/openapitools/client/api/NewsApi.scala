...
class NewsApi(baseUrl: String) {
   ...
   */
  def newsV1ArticleList(sourceIds: Seq[String], lang: Option[String] = None, categories: Seq[String], excludeCategories: Seq[String], toTs: Option[Int] = None
): Request[Either[ResponseException[String, Exception], GENERICRESPONSE], Any] =
    basicRequest
      .method(Method.GET, uri"$baseUrl/news/v1/article/list?source_ids=${ sourceIds }&lang=${ lang }&categories=${ categories }&exclude_categories=${ excludeCategories }&to_ts=${ toTs }")
   ...
   *   code 503 : Error (The 503 error occurs when there is an issue with one of our data sources and we can't even return a partial answer.)
   */
  def newsV1CategoryList(
): Request[Either[ResponseException[String, Exception], GENERICRESPONSE], Any] =
    basicRequest
      .method(Method.GET, uri"$baseUrl/news/v1/category/list")
      .contentType("application/json")
      .response(asJson[GENERICRESPONSE])
   ...
   */
  def newsV1SourceList(lang: Option[String] = None, `type`: Option[String] = None, status: Option[String] = None
): Request[Either[ResponseException[String, Exception], GENERICRESPONSE], Any] =
    basicRequest
      .method(Method.GET, uri"$baseUrl/news/v1/source/list?lang=${ lang }&type=${ `type` }&status=${ status }")
      .contentType("application/json")
      .response(asJson[GENERICRESPONSE])
