...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://data-api.cryptocompare.com");
    ...
    apiClient->apiKeys_api_key_header = NULL;
    apiClient->apiKeys_api_key_query = NULL;
...
, list_t *apiKeys_api_key_header
, list_t *apiKeys_api_key_query
) {
    apiClient_t *apiClient = malloc(sizeof(apiClient_t));
    if(basePath){
    ...
    apiClient->progress_data = NULL;
    apiClient->response_code = 0;
    if(apiKeys_api_key_header!= NULL) {
        apiClient->apiKeys_api_key_header = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_api_key_header) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_api_key_header, pairDup);
        }
    }else{
        apiClient->apiKeys_api_key_header = NULL;
    }
    if(apiKeys_api_key_query!= NULL) {
        apiClient->apiKeys_api_key_query = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_api_key_query) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_api_key_query, pairDup);
        }
    }else{
        apiClient->apiKeys_api_key_query = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    apiClient->progress_func = NULL;
    apiClient->progress_data = NULL;
    if(apiClient->apiKeys_api_key_header) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_api_key_header) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_api_key_header);
    }
    if(apiClient->apiKeys_api_key_query) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_api_key_query) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_api_key_query);
    }
    free(apiClient);
}
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        if (apiClient->apiKeys_api_key_header != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_api_key_header) {
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_api_key_query != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_api_key_query) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
